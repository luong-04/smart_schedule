...
<div x-data="{ activeTab: 10 }" class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4">
            <div class="flex bg-slate-100 p-1.5 rounded-[2rem] gap-1">
                ...
                <button @click="activeTab = {{ $grade }}" 
                    :class="activeTab === {{ $grade }} ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
    ...
    @foreach([10, 11, 12] as $grade)
    <div x-show="activeTab === {{ $grade }}" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform translate-y-4" x-transition:enter-end="opacity-100 transform translate-y-0">
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-blue-50 overflow-hidden">
            <div class="p-8 border-b border-slate-50 bg-slate-50/30">
                <h3 class="text-sm font-black text-slate-700 uppercase tracking-widest flex items-center gap-2">
                    ...
                    Chi tiết chương trình Khối {{ $grade }}
                </h3>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @php $configs = $groupedConfigs->get($grade) ?? collect(); @endphp
                        ...
                        <tr>
                            <td colspan="4" class="px-8 py-20 text-center">
                                <div class="flex flex-col items-center opacity-30">
                                    ...
                                    <p class="text-xs font-black uppercase tracking-widest">Chưa thiết lập định mức cho Khối {{ $grade }}</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
