...
@section('content')
    ...
    <style>
        ...
        @media print {
            ...
            body.print-single-mode #historyResult {
                display: block !important;
                width: 100% !important;
                max-width: 100% !important;
            }

            body.print-single-mode #printAllContainer {
                display: none !important;
            }

            body.print-all-mode #historyResult {
                display: none !important;
            }
    ...
    <div class="container mx-auto px-4 py-8" x-data="proctorManager()">
        ...
        <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 print-hidden">
            ...
            <div class="flex flex-col items-end gap-2">
                @if(session('success'))
                    <div
                        class="bg-green-50 border-l-4 border-green-500 text-green-700 px-6 py-3 rounded-xl shadow-sm flex items-center print-hidden">
                        <span class="font-medium">{{ session('success') }}</span>
                    </div>
                @endif
                @if($errors->any())
                    <div
                        class="bg-red-50 border-l-4 border-red-500 text-red-700 px-6 py-3 rounded-xl shadow-sm flex flex-col print-hidden">
                ...
                @if(session('error'))
                    <div
                        class="bg-red-50 border-l-4 border-red-500 text-red-700 px-6 py-3 rounded-xl shadow-sm flex items-center print-hidden">
        ...
        <div x-show="activeTab === 'setup'" style="display: none;" class="print-hidden">
            <form action="{{ route('admin.proctors.assign') }}" method="POST" hx-boost="false"
                class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                @csrf
                <input type="hidden" name="import_data" :value="JSON.stringify(proctors)">

                <div
                    class="col-span-1 bg-white p-6 md:p-8 rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 flex flex-col h-full">
                    ...
                    <div class="space-y-5 flex-grow">
                        <div>
                            ...
                            <input type="text" name="exam_name" placeholder="VD: Thi cuối kỳ 2"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-indigo-100 outline-none"
                                required>
                        </div>
                        <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="total_days" min="1"
                                    class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-indigo-100 outline-none"
                                    required>
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Phòng/Ngày</label>
                                <input type="number" name="rooms_per_day" min="1"
                                    class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-indigo-100 outline-none"
                                    required>
                            </div>
                        </div>
                        <div class="pt-4 border-t border-gray-100">
                            ...
                            <div class="space-y-3">
                                <label class="flex items-center space-x-3 cursor-pointer group">
                                    <input type="checkbox" name="constraint_dept" value="1" checked
                                ...
                                <label class="flex items-center space-x-3 cursor-pointer group">
                                    <input type="checkbox" name="constraint_room" value="1" checked
                                        class="w-5 h-5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 transition cursor-pointer">
                                    <span class="text-sm text-gray-600 group-hover:text-gray-900">Tránh lặp Phòng cũ</span>
                                </label>
                                <label class="flex items-center space-x-3 cursor-pointer group">
                                    <input type="checkbox" name="constraint_pair" value="1" checked
                                        class="w-5 h-5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 transition cursor-pointer">
                                    <span class="text-sm text-gray-600 group-hover:text-gray-900">Tránh trùng cặp Giám
                                        thị</span>
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="mt-8">
                        <button type="submit"
                            class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 px-4 rounded-xl shadow-md transition-all">
                            Xác nhận & Phân công
                        </button>
                    </div>
                </div>
                ...
                <div
                    class="col-span-1 lg:col-span-2 bg-white p-6 md:p-8 rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 flex flex-col h-full">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div>
                            ...
                            <label for="excelFile"
                                class="cursor-pointer bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-semibold shadow-sm">
                                📥 Import Excel
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 mb-6 bg-gray-50 p-4 rounded-xl border border-gray-200">
                        <input type="text" x-model="newProctorName" placeholder="Nhập tên giám thị..."
                            class="flex-1 px-4 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-indigo-100">
                        <input type="text" x-model="newProctorDept" @keydown.enter="addProctor()"
                            placeholder="Đơn vị / Khoa..."
                            class="flex-1 px-4 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-indigo-100">
                        <button type="button" @click="addProctor()"
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-100 sticky top-0 z-10">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Họ và Tên
                                        </th>
                                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Đơn vị
                                        </th>
                                    ...
                                    <template x-for="(proc, index) in proctors" :key="index">
                                        <tr class="hover:bg-indigo-50/50">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-800"
                                                x-text="proc.name"></td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"
                                                x-text="proc.department || 'Khác'"></td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <button type="button" @click="proctors.splice(index, 1)"
                                                    class="text-red-500 hover:text-red-700 font-bold p-1">✕</button>
                                            </td>
                                        </tr>
                                    </template>
                                </tbody>
        ...
        <div x-show="activeTab === 'history'" style="display: none;">
            <div
                class="bg-white p-6 md:p-8 rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 print-hidden">
                ...
                <div class="flex flex-col md:flex-row gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    <input type="text" id="searchExamInput" oninput="filterExams()"
                        placeholder="🔍 Gõ tên kỳ thi để tìm nhanh..."
                        class="w-full md:w-72 py-3 px-4 bg-white border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-indigo-200 text-gray-700 font-medium">

                    <div class="flex-grow flex items-center bg-white rounded-xl border border-gray-300 px-3">
                        <select id="examSelector" onchange="fetchExamDates()"
                            class="w-full py-3 bg-transparent border-none outline-none text-gray-700 font-medium cursor-pointer">
                ...
                <div id="examControls" class="hidden mt-6 space-y-4">
                    <div
                        class="flex flex-col md:flex-row gap-4 items-center bg-white border border-indigo-100 p-4 rounded-xl shadow-sm">
                        <div
                            class="w-full md:w-auto font-bold text-indigo-800 uppercase tracking-wide text-sm whitespace-nowrap">
                            <span class="bg-indigo-100 px-3 py-1 rounded-md">Ngày đang xem</span>
                        </div>
                        <div class="flex flex-wrap gap-2 w-full justify-start">
                            <button onclick="exportWord()"
                                class="bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white border border-blue-200 font-semibold py-2 px-5 rounded-lg transition-all">
                                📝 Tải Word
                            </button>
                            <button onclick="exportExcel()"
                                class="bg-emerald-50 text-emerald-700 hover:bg-emerald-600 hover:text-white border border-emerald-200 font-semibold py-2 px-5 rounded-lg transition-all">
                                📊 Tải Excel
                            </button>
                            <button onclick="printSingle()"
                                class="bg-gray-100 text-gray-700 hover:bg-gray-700 hover:text-white border border-gray-300 font-semibold py-2 px-5 rounded-lg transition-all">
                                🖨️ In / PDF
                            </button>
                        </div>
                    </div>
                    ...
                    <div
                        class="flex flex-col md:flex-row gap-4 items-center bg-amber-50 border border-amber-200 p-4 rounded-xl shadow-sm">
                        <div
                            class="w-full md:w-auto font-bold text-amber-800 uppercase tracking-wide text-sm whitespace-nowrap">
                            <span class="bg-amber-200 px-3 py-1 rounded-md">Toàn bộ Lịch thi</span>
                        </div>
                        <div class="flex flex-wrap gap-2 w-full justify-start">
                            <button onclick="exportWordAll()"
                                class="bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-5 rounded-lg shadow-md transition-all">
                                📝 Tải Word (Tất cả)
                            </button>
                            <button onclick="exportExcelAll()"
                                class="bg-teal-600 hover:bg-teal-700 text-white font-bold py-2 px-5 rounded-lg shadow-md transition-all">
                                📊 Tải Excel (Tất cả)
                            </button>
                            <button onclick="printAll()"
                                class="bg-orange-600 hover:bg-orange-700 text-white font-bold py-2 px-5 rounded-lg shadow-md transition-all">
                                🖨️ In Tất Cả
                            </button>
                            <div class="flex-grow"></div>
                            <button onclick="deleteExam()"
                                class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-5 rounded-lg shadow-md transition-all mt-2 md:mt-0">
                                🗑️ Xóa Lịch Này
                            </button>
                        </div>
                    </div>
            ...
            <div id="historyResult" class="hidden mt-8">
                <div id="printHeader" class="w-full">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 24px;">
                        <div style="width: 40%; text-align: center;">
                            <p class="formal-text" style="font-weight: normal; margin-bottom: 8px; white-space: nowrap;">BỘ
                                GIÁO DỤC VÀ ĐÀO TẠO</p>
                            <p class="formal-text" style="line-height: 1; margin-bottom: 8px;">
                                ........................................</p>
                            <p class="formal-text" style="line-height: 1;">........................................</p>
                        </div>
                        <div style="width: 5%;"></div>
                        <div style="width: 55%; text-align: center;">
                            <p class="formal-text" style="font-weight: bold; margin-bottom: 2px;">CỘNG HÒA XÃ HỘI CHỦ NGHĨA
                                VIỆT NAM</p>
                    ...
                    <div style="text-align: center; margin-top: 35px; margin-bottom: 20px;">
                        <h2 class="formal-title uppercase" id="printExamName" style="margin: 0;">BẢNG PHÂN CÔNG GIÁM THỊ
                        </h2>
                    </div>
                </div>
                ...
                <table id="mainTable" class="data-table">
                    ...
                    <thead style="display: table-header-group;">
                        <tr class="title-row" style="border: none !important; background: transparent !important;">
                            <td colspan="7"
                                style="border: none !important; text-align: right; font-style: italic; font-size: 11pt; padding: 4px 8px;">
                                <span style="color: #4b5563;">Kỳ thi:</span> <strong style="color: black;"
                                    id="repeatExamName"></strong> &nbsp;|&nbsp;
                                <span style="color: #4b5563;">Ngày:</span> <strong style="color: black;"
                                    id="repeatDate"></strong>
                            </td>
                        </tr>
                        <tr>
                ...
                <table class="signature-table"
                    style="width: 100%; border: none !important; margin-top: 30px; page-break-inside: avoid;">
                    <tr style="page-break-inside: avoid;">
                        ...
                        <td style="width: 40%; text-align: center; border: none !important; vertical-align: top;">
                            <p class="formal-text"
                                style="margin: 0; page-break-inside: avoid; text-align: center; color: black;">
                                <span style="font-weight: bold; font-size: 13pt;">Người phân công</span><br>
                                <span style="font-size: 12pt; font-style: italic; font-weight: normal;">(Ký, ghi rõ họ
                                    tên)</span>
                            </p>
