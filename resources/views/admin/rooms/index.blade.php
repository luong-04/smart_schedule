        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-blue-50 overflow-hidden flex flex-col h-full">
            <div class="p-6 border-b border-slate-50 flex justify-between items-center bg-white">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('room-types.create') }}" class="bg-blue-600 text-white p-2.5 rounded-xl shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all flex items-center justify-center">
            ...
            <div class="flex-1 overflow-x-auto">
                <table class="w-full text-left">
                    <thead class="bg-slate-50 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                        <tr>
                            ...
                            <th class="p-4 w-12 text-center">
                                <input type="checkbox" 
                                    @change="
                                        ...
                                        else selectedRoomTypes = selectedRoomTypes.filter(id => !ids.includes(id));
                                    "
                                    :checked="{{ $roomTypes->count() > 0 ? 'true' : 'false' }} && {{ $allTypeIds }}.every(id => selectedRoomTypes.includes(id))"
                    ...
                    <tbody class="divide-y divide-slate-50 text-xs">
                        ...
                        <tr class="hover:bg-blue-50/30 transition-colors group" :class="selectedRoomTypes.includes({{ $type->id }}) ? 'bg-blue-50/50' : ''">
                            <td class="p-4 text-center">
                                <input type="checkbox" value="{{ $type->id }}" x-model="selectedRoomTypes" 
                                       class="w-3.5 h-3.5 text-blue-600 rounded border-slate-300">
                            </td>
                            <td class="py-4">
                                <span class="font-black text-blue-700 uppercase tracking-widest">{{ $type->name }}</span>
                            </td>
                            <td class="p-4 text-right">
                                <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <form action="{{ route('room-types.destroy', $type->id) }}" method="POST" onsubmit="return confirm('Xóa loại phòng này?')">
                                        @csrf @method('DELETE')
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100">
                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">{{ $roomTypes->count() }} LOẠI PHÒNG</p>
            </div>
        </div>
    </div>
    ...
    <div class="col-span-12 lg:col-span-8 space-y-6">
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-blue-50 overflow-hidden flex flex-col h-full">
            <div class="p-6 md:p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-bold uppercase mt-1">Danh sách phòng dùng để xếp TKB</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    <button x-show="selectedRooms.length > 0" 
                        ...
                        <span class="material-symbols-outlined text-[16px]">delete_sweep</span> Xóa (<span x-text="selectedRooms.length"></span>)
                    </button>

                    <a href="{{ route('rooms.create') }}" class="bg-emerald-500 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-200 hover:bg-emerald-600 transition-all flex items-center gap-2">
            ...
            <div class="flex-1 overflow-x-auto">
                <table class="w-full text-left">
                            ...
                            <th class="px-8 py-5 w-12 text-center">
                                <input type="checkbox" 
                                    @change="
                                        ...
                                        else selectedRooms = selectedRooms.filter(id => !ids.includes(id));
                                    "
                                    :checked="{{ $rooms->count() > 0 ? 'true' : 'false' }} && {{ $allRoomIds }}.every(id => selectedRooms.includes(id))"
                    ...
                    <tbody class="divide-y divide-slate-50 text-sm">
                        ...
                        <tr class="hover:bg-blue-50/20 transition-colors group" :class="selectedRooms.includes({{ $room->id }}) ? 'bg-blue-50/40' : ''">
                            ...
                            <td class="px-8 py-5 text-right">
                                <div class="flex justify-end gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('rooms.edit', $room->id) }}" class="text-slate-400 hover:text-blue-600 font-black text-[10px] uppercase tracking-widest transition-colors flex items-center gap-1">
                                    ...
                                    <form action="{{ route('rooms.destroy', $room->id) }}" method="POST" onsubmit="return confirm('Xóa phòng này?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="text-slate-400 hover:text-red-500 font-black text-[10px] uppercase tracking-widest transition-colors flex items-center gap-1">
                        ...
                        <tr>
                            <td colspan="4" class="px-8 py-20 text-center">
                                <span class="material-symbols-outlined text-6xl text-slate-200 mb-3 block">sensor_door</span>
                                <p class="text-xs font-black uppercase tracking-widest text-slate-400">Chưa có dữ liệu phòng học</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="px-8 py-4 bg-slate-50/50 border-t border-slate-100">
                <p class="text-[9px] font-black text-slate-500 uppercase tracking-widest text-right">Dữ liệu thời gian thực: {{ $rooms->count() }} phòng đang quản lý</p>
            </div>
        </div>
    </div>
</div>
